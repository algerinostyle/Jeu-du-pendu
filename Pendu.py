...
print ("Salut, " + name, "amusez vous bien !")
print ("\n")

regles=open("regles.txt") #affiche les règles
regle=regles.read()
regles.close
print(regle)
...
def affichemot(mot) : #fonction qui affiche le mot et le nombre d'essai restants
    ...
    lettres = []    #variable de type str vide
    ...
    if reponse.upper()==mot.upper()and vies !=0 : #si on arrive à afficher le mot correctectement et que le nombre d'erreur est différent de 0 
        print("Vous avez deviné le mot","\'",mot.upper(),"\'","!") #alors on affiche que le joueur a gagné et le mot
        gagne=open("gagne.txt","r")
        w=gagne.read()                          
        gagne.close()
        print(w)
        ...
        if type(recommencer)==str:                               #grâce à cette boucle on peut rejouer directement
                mot=trouvermot()
                affichemot(mot)            
    if reponse.upper()!=mot.upper() and vies ==0: #mais si on n'yarrive pas alors le joueur a perdu
        print("Perdu le mot à trouver était","\'", mot.upper(),"\'")
        recommencer=input("Entrer une lettre pour recommencer")
        if type(recommencer)==str:
                mot=trouvermot()
                affichemot(mot)
...
def nbvies(mot,L,vies) : #fonction paramétrique du nombre d'essais en fonction du nombre d'erreur 
    if L.upper() in mot.upper() : #si la lettre proposé est dans le mot demandé alors le nombre d'essais reste le meme 
        ...
        vies=vies-1 #sinon si on fait une erreur le nombre d'essais diminue de un
